...
sub main {
	...
	my ($archiveFileName) = fileparse($archiveFile); # get short file name (wo path)
	...
	foreach my $key ( @keys ) {
		...
		my ( $mclLargeCluster, $minorClustersRef, $usedCutoff ) = 
...
#					$minorClusterData{'clusterText'} = "Minor cluster #".($i+1).", $clusterSize/$numOfRuns";
	...
	foreach my $largeClusterHashRef (@largeClustersData) {
    	...
    	if (index($distructOutput, "Error:") != -1) {
			...
			&WriteToFile( $imagesToDisplay, "K=$curK\t$clusterText\t$clusterName.png" );
    	}
	}
		...
		if ( exists $minorClustersDataByKey{$key} ) {
			...
			foreach my $minorClusterDataRef (@minorClustersData) {
    			...
    			if (index($distructOutput, "Error:") != -1) {
					...
					&WriteToFile( $imagesToDisplay, "K=$curK\t$clusterText\t$minorClusteName.png");
    			}
			}
		}
	}
